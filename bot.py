...
async def handle_index(request):
    try:
        ...
        return web.Response(text="<h1>Файл index.html не найден</h1>", content_type='text/html', status=404)
...
async def run_http_server():
    app = web.Application()
    app.router.add_get('/', handle_index)          # Главная страница (Mini App)
    app.router.add_get('/health', handle_healthcheck)  # Для Railway healthcheck
